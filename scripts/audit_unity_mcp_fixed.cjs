...
const results = [];
for (const name of ['transport-regression.cjs', 'index-freshness-regression.cjs', 'launcher-regression.cjs', 'probe-support-regression.cjs', 'diagnostic-freshness-regression.cjs', 'evidence-regression.cjs', 'source-ast-regression.cjs', 'evals~/harness.test.cjs', 'legacy-suites']) {
  ...
  fs.writeFileSync(path.join(out, name.replace(/[\\/]/g, '-') + '.log'), (r.stdout || '') + (r.stderr || ''));

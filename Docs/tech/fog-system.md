...
- **Noise Scroll = 풍속**: 안개가 그 방향으로 흐른다(현재 0.3/0.15). 0으로 두면 정지.
- 노이즈는 **로컬 볼륨 경계도 흩뜨려**(도메인 워프) 큐브/원 형태를 숨긴다(§4).
- 즉 **기획이 텍스처를 안 줘도 동작**하고, 원하면 텍스처로 교체해 룩을 바꿀 수 있음.

---

## 4. FogVolume (로컬 영역 포그 — 추가 / 제거)
오브젝트(또는 존 프리팹 루트)에 `Fog Volume` 추가. 영역에 포그를 **추가(+)** 하거나 **제거(−, 클리어링)** 한다.
| 항목 | 의미 |
|------|------|
| **Shape** | `Box`(직육면체) / `Sphere`(원형) |
| **Box Size / Sphere Radius** | 영역 크기(Transform 스케일과 곱해짐) |
| **Density** | **양수 = 포그 추가 / 음수 = 포그 제거(클리어링)**. 예: -1 = 그 영역 포그를 걷어냄 |
| **Soft Border** | **경계 페이드 폭(미터)**. 클수록 부드럽게 사라짐. 원형 클리어링은 반지름의 절반~비슷하게(예: 반경 15 → 7~12) |
| **Override Color / Color** | 그 영역만 다른 색(예: 독구름=초록) |

- **경계는 노이즈로 자동 도메인 워프** → 큐브/원 형태가 그대로 드러나지 않고 **유기적으로 흩어진다**(노이즈 ON일 때).
- **"포그가 자연스럽게 줄어드는 원형 영역"** = `Shape: Sphere` + `Density: 음수` + `Soft Border` 넉넉히 + 노이즈 ON.
- Transform로 배치하므로, 맵 존 프리팹에 붙이면 **존이 셔플돼도 포그가 따라감**(테마별 포그).
- 거리순 최대 16개 활성(FogManager `Max Volumes`).
...
## 6. 층 디밍 (전장의 안개) — PC 전용
> 플레이어 y 기준 위/아래로 일정 범위를 벗어난 픽셀의 **채도·명도를 낮춰 거의 검정**으로 만든다.
> 다른 층을 어둡게 가려 "현재 층"에 집중시키는 연출. **포그와 독립 토글**(같은 풀스크린 패스에서
> 처리 → 추가 GPU 패스 0). 가우시안 블러가 아니라 per-pixel desaturate + darken.

기준선은 **플레이어 y 상대** — 계단/엘리베이터로 올라가면 기준선도 따라간다.
**PC 전용**(Mobile_RPAsset 은 Depth Texture 비활성이라 미적용).

### 켜는 법
1. PC_Renderer 에 `Fog Renderer Feature` 등록(0절과 동일 — 포그와 같은 패스를 공유).
2. FogManager 의 **Dim Enabled** 체크. (포그를 안 써도 디밍만 단독 사용 가능)
3. 기준 플레이어 자동 탐색: `CameraTargetSwitcher` 의 현재 추적 타겟 → `"CameraFollowTarget"` 태그 순.
   수동 지정하려면 FogManager 의 **Dim Player Override** 에 Transform 할당.

### 파라미터 (FogProfile)
| 항목 | 의미 |
|------|------|
| **Dim Range Up / Down** | 플레이어 y 기준 위/아래 허용 범위(m). 넘으면 디밍 시작(위/아래 비대칭 가능) |
| **Dim Fade Up / Down** | 위/아래 페이드 폭(m). 클수록 완만하게 어두워짐 |
| **Dim Saturation** | 완전 디밍 시 채도 잔량(0 = 완전 흑백) |
| **Dim Brightness** | 완전 디밍 시 명도 곱(0 ≈ 검정, 권장 0~0.1) |
| **Dim Affect Sky** | 하늘(원거리) 픽셀에 적용할 비율(0 = 하늘 제외 권장) |

> 토글: `Dim Enabled` 만 끄면 디밍만 사라지고 포그는 유지(반대도 동일). 둘 다 끄면 패스 자체가 스킵된다.
> 포그+디밍 동시 적용 시 **포그를 먼저 깔고 그 위에 디밍**(안개 낀 색을 다시 어둡게)하는 순서로 한 패스에서 처리.

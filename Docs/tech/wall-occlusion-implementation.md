...
이 시스템은 각 로컬 게임 카메라에서 카메라와 플레이어 사이를 실제로 가리는 등록 구조물만 선택한다. 선택된 Renderer의 원본 머티리얼과 원본 셰이딩은 그대로 유지하고, 원본 Shader Graph에 통합된 디더 컷아웃만 활성화한다. 화면에 투영된 플레이어 기본 충돌 캡슐의 중심은 완전히 비우고 바깥 테두리만 디더 그라데이션으로 연결한다.

고도 구조에서는 현재 Level보다 높은 Level의 등록 Collider가 SphereCast 후보에 들어오고 화면 캡슐 표본 시선을 실제로 차단하면 그 Level의 `Content` 전체를 같은 마스크로 처리한다. 현재 Level과 아래 Level은 그룹 처리하지 않고 확인된 `OcclusionSection` 차단만 개별 처리한다.

최종 활성 조건은 다음 하나다.

```text
LevelAboveAndConfirmedBlock || ConfirmedSectionBlock
```
...
5. 카메라에서 캡슐 중심까지 `Physics.SphereCastNonAlloc`을 한 번 실행해 1차 후보를 수집한다.
6. 화면 캡슐 안쪽의 11개 표본 시선에 후보 Collider가 실제로 교차하는지 `Collider.Raycast`로 확인한다.
7. 확인된 Collider만 `WallOcclusionRegistry`에서 Level 소속과 Section 소유로 조회하고 Renderer 합집합을 만든다.
8. `WallOcclusionRendererController`가 원본 머티리얼은 바꾸지 않고 per-renderer 전환 강도만 적용하며, 해제 완료 시 기존 property block을 복원한다.

SphereCast와 표본 시선 배열은 고정 배열을 재사용한다. 표본은 캡슐 축의 양 끝 중심과 내부 세 지점의 중심·좌·우로 구성한다. SphereCast 버퍼가 가득 차면 세션 중 한 번 경고하고 `WallOcclusionSettings.maxCastHits`를 늘리도록 안내한다. Trigger와 플레이어 자신의 Collider는 제외한다.
...
공통 `WallOcclusionClip.hlsl`을 실제 사용 중인 네 원본 Shader Graph가 직접 호출한다. 월드 위치를 gameplay camera 화면 좌표로 다시 투영하므로 Forward, DepthOnly, DepthNormals, ShadowCaster가 같은 마스크를 사용한다. 중심 반경은 완전히 clip하고 바깥 feather에서만 디더 임계값이 변화한다. 플레이어보다 뒤쪽인 픽셀은 깊이 제한으로 불투명하게 보호한다.

적용 그래프는 `Generic_Standard`, `Generic_Basic`, `ConvayorBelt_Graph`, `ConvayorBelt_Corner_Graph`다. 기존 Alpha Clip 재질은 기존 `BaseAlpha - BaseThreshold`와 가림 디더 margin 중 작은 값을 최종 clip 조건으로 사용한다. 따라서 원래 알파 컷아웃과 색·노멀·금속성·Emission 계산은 유지된다.

feather 폭은 고정 픽셀이 아니라 화면에 투영된 플레이어 캡슐 반경에 `Feather Radius Scale`을 곱해 계산한다. 인스펙터에서는 0~3배까지 조절할 수 있고 `Min/Max Feather Pixels`로 최종 폭을 제한한다. 프로젝트 설정 에셋의 값은 플레이테스트를 통해 조정하며, 중심의 완전 컷아웃 반경과는 독립적이다.
...
`WallOcclusionSettings`에는 재질 쌍이나 변형 재질 목록이 없다. 등록 대상의 모든 머티리얼이 `_WallOcclusionStrength` 속성을 지원하는지만 검사한다. 이름 기반 바인딩과 호환 모드는 없다.

선택된 Renderer의 `sharedMaterials`는 활성화·유예·복원 전 구간에서 한 번도 바꾸지 않는다. `MaterialPropertyBlock`의 `_WallOcclusionStrength`만 0→1→0으로 전환한다. 완전 활성 상태와 hit 해제 유예 중에도 강도 1을 유지하고, 강도가 0에 도달했을 때만 원래 property block을 복원한다.
...
Renderer의 머티리얼 슬롯 중 하나라도 `_WallOcclusionStrength`를 지원하지 않으면 해당 Level/Section 전체를 안전하게 불투명으로 제외하고 소유자 기준 한 번만 경고한다.
...
도구는 선택한 프리팹 에셋 또는 프리팹 인스턴스만 처리한다. Stage/Zone에서는 `Occlusion` 하위 Renderer만 대상으로 본다. 변형 머티리얼을 만들거나 교체하지 않으며, 원본 머티리얼의 디더 속성 지원 여부만 검증한다. 중첩 프리팹 원본은 수정하지 않으며 원본을 먼저 등록하도록 오류를 낸다.

검증 항목은 고정 이름, 필수 노드, 직속 프랍 루트, 중첩 `PF_Prop_*`, 컴포넌트 위치, 중복 Collider 소유, 다중 Level 소속, 기준 Y 중복, transform 제한, 최소 Renderer/Collider, XZ Area, 원본 머티리얼의 `_WallOcclusionStrength` 지원이다. 오류는 프리팹 경로와 전체 하이어라키 경로를 출력하지만 빌드를 막지 않는다.
...
| MPB 강도 전환·복원 | `Assets/1.Scripts/Rendering/Occlusion/WallOcclusionRendererController.cs` |
...
| 공통 디더 함수 | `Assets/3.Materials/Level1_Materials/Occlusion/WallOcclusionClip.hlsl` |
...
- EditMode 테스트: XZ 회전, Level/Section 이중 등록, 접지 Level 우선, 최초 20%, 상승 20%, 공중 상승 유지, 하강 60%, 영역 이탈, shared materials 무변경과 기존 property block 복원.

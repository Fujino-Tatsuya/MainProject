...
## 🔴 0. 구조 재설계 진행 중 (2026-08-06 저녁)

**팀장 지시로 상태 집합이 바뀐다.** 이 문서의 §1.1·§4 는 아래에 맞춰 재작성 예정이다.

| | 이 문서 초판 | **확정된 방향** |
|---|---|---|
| FSM 상태 | `TwentyThreeState` 15개 단일 enum | **`Idle` / `Walk` / `Attack` / `Hit` / `Groggy` / `Dead` 6개** |
| 공격 표현 | 공격마다 상태 1개 | **`Attack` 하나 안에서 서브타입**으로 |
| 근거 | 상태↔애니 1:1 이라 디버깅 쉬움 | **플레이어·일반 몬스터와 같은 규약으로 보스도 "찍어낼 수 있게"** |

이유: 지금 보스는 BT 기반이라 **플레이어/일반 몬스터를 보고 따라 만들 수가 없다.**
목표는 "일반 몬스터를 찍어내듯 보스도 찍어내는" 구조다.

⚠️ **FSM 상태(6개)와 애니메이터 `State` Int(15값)는 다른 층이다.**
애니메이터는 이미 15값으로 돌고 있으므로(§1.3), FSM 은 `(상태, 공격 서브타입)` → 애니 파라미터
**매핑 계층**을 갖는다. 애니메이터를 6개로 줄이는 게 아니다.

**추가 확정 범위**

- 🔴 **레이어 전면 정리** — 보스와 일반 몬스터를 다른 사람이 구현해 레이어 정리가 서로 다르다.
  본체 / 허트박스 / 공격 히트박스 / 투사체를 **일관된 표준**으로 맞춘 뒤에 구현에 들어간다.
- 🔴 **hit 리액션 애니메이터 스테이트 신규** — `getowned` 클립은 FBX 에 있으나
  `TwentyThreeController` 에 스테이트가 없다.

**조사 진행 중** — 플레이어 규약 / 일반 몬스터 규약 / 레이어 표준 3개 레인.
결과가 나오면 §1.1·§2·§4 를 그에 맞춰 갈아엎는다.
...
OnCounterSuccess():
    ...
    PlayHitReaction()                # ← 회의 확정(2026-08-06): Boss_23_getowned 재생
    GroggyCount += 1
    SetState(GroggyCount >= maxGroggyCount ? Break : Groggy)   # hit 재생 후 전이
```
...
### 10.5.1 폭탄 = **수평 당구 모델** (팀장 결정 2026-08-06)

> 초안에서 "수동 보간 유지"를 권했는데 **틀린 판단이었다.** 보스는 서버 권한 +
> `NetworkTransform` 복제라 클라가 물리를 재현할 필요가 없다. 물리로 가는 게 맞고,
> 그러면 수동 스윕(`CheckHitBetween`)이 통째로 사라진다.

**중요 — 이건 "중력 포물선"이 아니라 "수평 당구"다.**

| 항목 | 확정 |
|---|---|
| **y 축** | **변하지 않는다.** 지금의 포물선(`4·h·t·(1−t)`)을 버린다 |
| **이동** | 방향벡터를 받아 **그 방향으로 일직선**으로 쭉 나간다 |
| **회전** | **무회전.** 굴러가는 연출 없음 |
| **폭탄끼리** | 🔴 **당구처럼 서로 부딪히고 서로 밀린다.** 날아간 폭탄이 정지해 있는 폭탄을 치면 그 폭탄도 밀려난다 |

즉 폭탄은 **바닥 평면 위의 당구공**이다. 이게 이 기믹의 핵심 재미이므로 구현이 여기에 맞춰져야 한다.

**Rigidbody 설정**

```
useGravity   = false                       // 중력 안 씀 (y 고정)
constraints  = FreezePositionY | FreezeRotation
collisionDetectionMode = ContinuousDynamic // 되쳐낸 폭탄은 빠르다 — 관통 방지
isKinematic  = 클라에서 true               // NetworkRigidbody 가 자동 처리
```

- **폭탄끼리 충돌하려면 둘 다 논키네마틱 Dynamic 이어야 한다**(서버 기준).
  정지한 폭탄을 `isKinematic` 으로 재워두면 **당구가 안 된다** — 정지해도 Dynamic 을 유지하고
  `Sleep()` 에 맡긴다(충돌하면 자동으로 깬다).
- **무회전은 `FreezeRotation`** 으로. `angularDrag` 로 잡으려 하면 미세하게 돈다.

**되쳐내기**

```
dir = normalize(bomb.pos - player.pos)     // 수평 성분만, y = 0
rb.AddForce(dir * damage * knockCoef, ForceMode.Impulse)
```

`knockCoef` 는 **SO 노출** — 지금은 `distance = damage` 로 계수 없이 하드코딩돼 있다.

**"정지해야 터진다"의 판정**

`OnCollisionEnter` 만으로는 안 된다 — 부딪히고도 계속 밀려간다.
→ `velocity.sqrMagnitude < ε` 가 **일정 시간 유지**되면 정지로 본다(`rb.IsSleeping()` 병용).
비행 중에는 폭발 타이머가 멈추고, 정지 판정이 서면 재개한다(기획 그대로).

**그 외**

- **보스는 안 밀린다** — 폭탄이 유닛 Rigidbody 에 힘을 주지 않게 한다. 유닛·벽 충돌은 어차피
...
| **A3 피격 리액션** | ✅ **회의 확정(2026-08-06).** 카운터 성공 = **돌진 또는 그랩을 취소 → hit 애니메이션 출력 → `Groggy`**. 즉 **카운터 전용**이고 일반 피격에는 적용하지 않는다 |
| **A4 초기 투척 궤적** | ⏸ Wells 가 **처음 던질 때**도 포물선 없이 수평인가? 손 높이에서 바닥으로 내려오는 구간이 필요해 보인다. 현재 가정 = **투척 직후 바닥 높이로 내려온 뒤 그때부터 수평 당구** |

**A3 회의 결과** — 카운터 성공 경로에 **hit 리액션이 한 단계 들어간다**:

```
카운터 적중 → 돌진/그랩 취소 → Boss_23_getowned 재생 → Groggy (또는 Break)
```

일반 피격에는 적용하지 않는다 — "보스는 아무 공격에나 경직되지 않는다"는 원칙 유지.
따라서 공격 선택기·쿨다운·슈퍼아머 설계는 바뀌지 않는다.
⚠️ 애니메이터에 **hit 리액션 스테이트가 없다** — `getowned` 클립은 FBX 에 있지만
`TwentyThreeController` 에 스테이트가 없다. **애니메이터 작업이 새로 필요하다**(지금까지
"애니메이터는 안 고쳐도 된다"고 했는데 이 한 가지는 예외다).

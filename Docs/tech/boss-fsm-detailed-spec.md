...
## 🔴 0. §1.1 과 §4 는 폐기됐다 — [boss-rebuild-standard.md](boss-rebuild-standard.md) 를 보라

2026-08-07 **독립 4레인 교차 조사**(Claude opus ×2 · Codex `gpt-5.5` · Codex `gpt-5.6-sol`)로
구조가 확정됐다. 이 문서의 아래 두 섹션은 **더 이상 유효하지 않다**:

| 섹션 | 초판 | 확정 |
|---|---|---|
| §1.1 | `TwentyThreeState` 15개 단일 enum | ❌ 폐기 → **`BossState` 6개 + `AttackId` + `AttackPhase` 3층** |
| §4 | 15상태 전수표 | ❌ 폐기 → 표준 문서 §4 |

**이유**: 중간보스 `WallBot` 은 **C# 코드가 0줄**이고 `MonsterBase` + SO 값만으로 만들어졌다.
중간보스 3종 중 **어느 것도 `MonsterState` 에 값을 추가하지 않았다.** 공격은 `Attack` 상태
하나에서 타입으로 분기하는 것이 이미 확립된 규약이다. 초판의 15상태 설계가 규약 위반이었다.

**이 문서에서 여전히 유효한 것** (표준 문서에 없는 세부):

- §1.3 **애니메이터 계약·클립 21개 실측** — 그대로 유효. `State` Int(15값)는 애니메이터 층이고
  FSM 6상태와는 별개다(매핑 계층을 둔다)
- §6 애니 이벤트 배치 · §7 JumpAttack 시퀀스 · §8 카운터 창
- §9 페이즈·송전기(`==` 교착 버그 포함) · §10 Wells FSM · **§10.5 폭탄/장판**
- §11 데이터 스키마 · §12 미확정 질문

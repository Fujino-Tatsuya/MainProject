...
| # | 질문 | 제 의견 |
|---|---|---|
| **L1** | **`Weapon(12)` 을 "공격 히트박스 앵커"로 재정의하고 이관할까요?** | **찬성.** §3.1 이 실제 버그를 냈고(폭탄이 보스 공격 콜라이더를 바닥으로 오인), 이관하면 그 버그 클래스가 구조적으로 사라집니다. 지금 `Weapon` 은 0건 사용이라 충돌 위험도 없습니다 |
| **L2** | **`17 CombatTarget`(신규)** 을 만들어 송전기·차징 기둥처럼 **`Unit` 은 아닌데 공격 가능한 대상**을 `Enemy(8)` 에서 분리할까요? | **보류 권장.** 지금 기둥이 `Enemy` 라서 보스 타겟팅과 섞일 여지는 있지만, 실제 오작동이 관측된 적은 없습니다. 레이어 신설은 되돌리기 어려우니 **보스 재작성 후 실제 문제가 나오면** 하는 게 낫습니다 |

...
1. 🔴 **Play 검증** — 아래 「Play 체크리스트」. 여기서부터 시작한다(코드·배선·자동검증은 끝났다)
2. ~~WallBot 인터럽트 추가~~ — **코드 + 프리팹 배선 완료**(§4). Play 검증만 남았다
...
- [ ] **WallBot 3단**: 방패 모으고 멈춰 있다(1.5초) → 인터럽트 안 하면 **전방 돌진**(경로 전원 데미지+넉백)
      → 도착해서 **2단 애니 + 주변 충격파**. 인터럽트 성공하면 Hit 자세로 그로기 0.5초
- [ ] WallBot 이 모으는 도중에 **근접 판정이 새지 않는지**(창 동안 히트를 막았다 — 새면 override 가 안 먹은 것)
- [ ] 근접 3종 개시 거리 수정 반영: 최대 거리에서 때려도 **판정이 붙는다**(Spinner 2.0 · Wall 2.0 · Gauntlet 3.2)

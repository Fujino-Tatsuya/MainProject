...
0. 🔴 **가장 먼저 — Play 를 끄고 배선 한 번 누르기.** 아래 「Play 종료 직후 해야 하는 것」
1. **Play 재확인** — 아래 「Play 체크리스트」(WallBot 3단 공격 항목 포함)
2. ~~WallBot 인터럽트 추가~~ — **코드 완료**(§4). 프리팹 배선만 남았다
3. **중간보스 어그로 확인** — 재선정 8초가 2인에서 실제로 도는지(MPPM `MidBossTest`)
4. 검증 끝나면 **팀원 머지 반영** → 5. **SVN 최신화**

## 🔴 Play 종료 직후 해야 하는 것 (2026-09-08 세션 마지막 상태)

세션 종료 시점에 에디터가 **Play(일시정지)** 였다. 그 상태에서는 **스크립트 컴파일과 에셋 임포트가
보류**되므로 아래가 전부 "코드는 커밋됐지만 아직 컴파일·배선되지 않은" 상태다.

1. **Play 종료** → 컴파일 확인(`WallBot.cs`·감사 도구 2개가 처음 컴파일된다. 🔴 **컴파일 검증을 못 한 채
   커밋했다** — 에러가 나면 그 자리에서 고쳐야 한다)
2. `Tools/Boss/중간보스 — 인터럽트 카운터 배선 (멱등)` **한 번 누르기** — WallBot 스크립트 교체 +
   창 부착 + 데이터 정리까지 한다. 로그에서 필수 참조 3개가 `O` 인지 볼 것

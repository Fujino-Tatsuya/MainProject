...
1. **모으기** — 방패 두 개가 모이는 지점에서 **1.5초 인터럽트 창**
2. **실패 → 전방 돌진** — 돌진 경로/범위 안의 플레이어 **전원 데미지 + 넉백**
3. **도착 → 공격 애니 재생 + 주변 충격파 + 넉백**

→ `MonsterBase` 를 그대로 쓰는 현재 구조로는 안 되므로 **`WallBot` 전용 클래스 신설**.
⚠️ 프리팹의 스크립트를 교체하면 `MonsterBase` 로 직렬화된 인스펙터 값(참조 6개 + 레이어마스크)이 끊긴다 —
교체 전에 값 목록을 적어 두고 새 클래스에 같은 필드명으로 옮긴다.

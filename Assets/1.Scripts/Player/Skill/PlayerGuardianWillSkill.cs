...
/// </summary>
public class PlayerGuardianWillSkill : PlayerInstantSkill
{
    ...
    private GuardianWillSkillData GuardianData => Data as GuardianWillSkillData;

    public override void OnServerStart(Vector3 direction, Unit target)
    {
        base.OnServerStart(direction, target);

        GuardianWillSkillData data = GuardianData;
        if (data == null)
        {
            Debug.LogError("[Player] 수호자의 의지에는 GuardianWillSkillData가 필요합니다.", this);
            return;
        }

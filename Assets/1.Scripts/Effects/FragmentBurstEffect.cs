...
/// <b>파편 충돌을 아무 데나 열면 안 되는 이유</b>이기도 하다 — 파편이 플레이어를 미는 결과가
/// 클라마다 갈리면 그건 연출이 아니라 디싱크다.
///
/// 그래서 콜라이더는 <b>월드 지오메트리에만</b> 반응한다(2026-09-21). Bake가 파편을 Effect 레이어에
/// 올리고 <c>Collider.excludeLayers</c>로 Default·Ground·Wall·Env 를 제외한 전부를 막는다 —
/// 플레이어·적·모든 hurtbox는 물론 <b>파편끼리도</b> 통과한다(맞닿은 채 시작하므로 서로 밀면 폭발이 튄다).
/// 레이어 충돌 매트릭스는 팀 공용 파일이라 건드리지 않았다.
/// 바닥을 뚫지 않으려면 <c>collisionDetectionMode</c>도 ContinuousSpeculative 여야 한다 —
/// 파편은 작고 빨라서 Discrete 로는 얇은 바닥을 그냥 통과한다.
///

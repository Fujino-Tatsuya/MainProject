...
public class GauntletBot : MonsterBase
{
    ...
    void CounterSucceeded()
    {
        ...
        // 🔴 장판을 여기서 따로 끄지 않는다(2026-09-09 머지). 예전에는 HideTelegraphClientRpc() 를
        //    불렀는데, 민경님 전환 후에는 아래 ForceGroggy → SetState(Groggy) → PlayStateAnimation
        //    에서 `s != Attack` 이면 smashTelegraph.Stop() 이 돌아 **이미 꺼진다.**
        //    두 번 끄면 안 되는 건 아니지만, 끄는 지점이 둘로 갈리면 한쪽만 고쳐 어긋난다.

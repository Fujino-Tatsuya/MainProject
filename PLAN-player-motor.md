...
| | 내용 | 성격 |
|---|---|---|
| **4a** | 시뮬레이션을 순수 함수로 추출 + 재생 경로에서 부작용 분리 + 결정론 EditMode 테스트 | **리팩터** — 동작 변화 없음 |
| **4b** | 틱 번호·입력 버퍼 · 서버 권위 시뮬레이션 · 예측/재조정 | **기능 추가** |

4a는 "감각 동일 + 결정론 테스트 통과"로 검증이 끝나므로 안전하다. 4b가 실제 네트워크 작업이다.

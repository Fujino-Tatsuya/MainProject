...
오너가 예측으로 움직여봐야 지워지고, 오너만 제외하는 옵션이 없다.
...
**"클라가 스폰 직후 이동을 못 한다" 증상의 1순위 용의자다.** b2 가 이 모순을 구조적으로 제거한다.
...
| **4b2** | `NetworkTransform` 제거 + 자체 복제(서버→피어, 원격 보간) · `IsMovementAuthority` 분해 | **네트워크 구조 변경** | 착수 (2026-09-15) |
| **4b3** | 예측 · 되감기 · 재생 · 보정 적용 (임계값은 b1 실측으로 확정) | **기능 추가** | 대기 |
...
### 4b2 상세 설계 (2026-09-15) — α / β 로 다시 쪼갠다

b2 는 이 계획에서 가장 위험한 구간이라 **되돌아갈 수 있는 중간 지점**을 하나 만든다.
핵심 관찰: **`NetworkTransform` 은 이미 서버 권위다.** 그러니 "서버가 실제로 시뮬레이션하게"
만드는 것만으로 NT 를 건드리지 않고 서버 권위가 완성된다. NT 제거는 그 다음 문제다.

| | 내용 | 얻는 것 | 잃는 것 |
|---|---|---|---|
| **b2-α** | 권한 개념 분해 + **서버가 전 플레이어를 시뮬레이션해 커밋**. NT 는 그대로 둔다 | 서버 권위 성립. **클라 이동 불가 버그 소멸**(서버가 실제로 움직이므로) | 오너 예측이 NT 에 덮여 **입력이 RTT 만큼 늦게 보인다** — 감각 저하. β 까지가 한 세트 |
| **b2-β** | 루트 NT 제거 + 자체 복제(서버→피어) + 원격 보간 | 오너 로컬 예측이 안 지워진다. 감각 회복 | 보간을 직접 짜야 함 |
...
**4. 범위 밖.** NT 제거·자체 복제·원격 보간·되감기/재생/보정은 전부 β 와 b3.

#### b2-β (α 검증 후 착수)

- 루트에서 `NetworkTransform` 제거. 자식에 붙은 NT 가 있는지 먼저 조사.
- 서버 → 비서버 피어 **unreliable RPC** 로 스냅샷: `tick`, `position`, `armatureYaw`, `flags(grounded/teleport)`.
  최신값만 의미 있고 손실 허용이므로 `NetworkVariable` 보다 unreliable 이 맞다.
- 전송 레이트 = 네트워크 틱. 물리 50Hz 전부 보내지 않는다.
- **원격 피어**: 스냅샷 버퍼 + 고정 지연(스냅샷 2개분) 렌더. 위치 `Lerp`, yaw `Mathf.LerpAngle`.
  버퍼 고갈 시 마지막 속도로 최대 200ms 외삽 후 정지.
- **오너**: 스냅샷을 **저장만** 한다(b3 의 되감기 기준점). 단 `teleport` 플래그는 즉시 적용 —
  부활·보스 그랩·텔레포트는 예측 대상이 아니다. 기존 `SetPoseTarget` 경로에 연결.

...
착수 전 정할 것:
1. **보정 임계값** — 몇 cm부터 되감을 것인가. 너무 작으면 상시 보정(끊김), 너무 크면 벽 통과가 남는다.
2. **보정 적용 방식** — 순간 이동인가, 몇 틱에 걸친 부드러운 수렴인가. 탑다운 협동에서는 후자가 보통이다.
3. **입력 버퍼 길이** — RTT 상한을 얼마로 잡는가(현재 대시 스냅샷은 `snapshotCapacity 32` = 0.64s).
4. **`PlayerDashValidationManager`를 흡수할지, 병존시킬지** — 흡수가 목표였지만 대시는 무적·충전
   장부까지 얽혀 있어 범위가 커진다. 이동 재조정을 먼저 세우고 대시는 그다음이 안전하다.

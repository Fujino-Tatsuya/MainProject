...
현재 `Player.prefab` 루트는 `AuthorityMode: 1`(Owner)이다. 이걸 Server로 바꾸는 것만으로는
안 된다 — NGO 기본 `NetworkTransform`은 서버 권위 모드에서 **오너 클라의 transform도 매 프레임
덮어쓴다.** 오너가 예측으로 움직여봐야 지워지고, 오너만 제외하는 옵션이 없다.
...
| **4b1** | 틱 번호 · raw 입력 분리 · 입력/상태 링버퍼 · 서버 병행 시뮬레이션 + `[Recon]` 관측 로그 | **동작 변화 0** | 진행 중 |
| **4b2** | `NetworkTransform` 제거 + 자체 복제(서버→피어, 원격 보간) · `IsMovementAuthority` 분해 | **네트워크 구조 변경** | 대기 |

...
- ✅ 팀장 확인(09-24): **벽 모니터 13대 화면 반복**(TitleScreenFeed, 백화 없음) · **메인 모니터 Idle = Re:C 로고 CRT + 상시 글리치**
  (원본 유리 머티리얼의 스카이박스 반사 제거) · 카메라 도착 순간 메뉴 전환.

### 0.9 🟡 미결 — 다음 세션에서 결정 (팀장 09-24)
- **Start 후 로비 진입이 너무 급하다.** 꺼짐(1.1초) 직후 `3.LobbyScene` 이 바로 뜬다.
  → 로비 진입 때 **CRT 켜짐 연출**(가로 띠 → 펼쳐짐, 꺼짐의 역재생 계열)을 넣을지 고민 중. 결정 전에는 손대지 않는다.
  - 넣는다면 후보: (a) 로비 씬 첫 프레임을 검정으로 시작해 `Title/CRTOff` 를 역방향 재생(캡처 대신 로비 카메라 화면을 대상으로 — Renderer Feature 또는 로비 전용 오버레이 필요)
    (b) 단순히 검정 유지 시간 + 짧은 페이드 인. 🔴 로비는 타이틀과 다른 씬이라 타이틀 쪽 컴포넌트가 살아 있지 않다 — 로비 측 구현이 필요하다.
- 남은 단계: 3단계 설정창 가독성(현재 0.78배 축소) · 8단계 회귀/도구 동기화.
